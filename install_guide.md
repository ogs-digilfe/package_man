...
# はじめに
以下、pjフォルダを./として記載する。  
1.1.で仮想環境を構築した場合は、すべて仮想環境でコマンドを実行する前提とする。


# １．環境構築
## 1.1. os環境の設定
### 1.1.1　venvの作成(オプション)
mkdirコマンドでアプリケーションのpjフォルダを適当な場所に作成し、cdコマンドで作成したpjフォルダに移動。  
pjフォルダのトップにpython仮想環境(venv)を作成する。  
詳細は、pythonのドキュメントを参照のこと。
...
step1: pjフォルダからsourceコマンドで以下のファイルを読み込んで実行し、作成した仮想環境に入る  
...
apache airflowのルートフォルダ./airflowで、airflowコマンドを使ってapache airflowを初期化する。

\$ cd ./airflow  
\$ airflow db migrate

## 1.5. apache airflowが参照する環境変数AIRFLOW_HOMEのセット
apache airflowは、どこがapache airflowのルートフォルダかを参照する際、環境変数AIRFLOW_HOMEを参照するようになっているため、exportコマンドで環境変数AIRFLOW_HOMEをセット。  
__"./"は正しいpjフォルダのフルパスに書き換えること。__

\$ export AIRFLOW_HOME=./airflow

次回以降、環境変数AIRFLOW_HOMEがシステム起動時などに自動セットされるよう、適切なファイルに環境変数AIRFLOW_HOMEの設定を追記。  
ubuntuの場合は、/etc/environmentに追記。  
__"/path/to/project/は正しいpjフォルダのフルパスに書き換えること。__
...
※)同期されるコンテンツフォルダは以下の自作コードのフォルダのみ。
DAGが保存されている  
./dag/
...
playbookフォルダに移動して、inventoryファイルを編集する。
...
inventoryファイルの編集が終わったらplaybookフォルダに移動して、接続確認を実施。
