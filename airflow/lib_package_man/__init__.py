...
class MakePlaybook():
    ...
    # 引数で指定したpackageがinstallされたhostを抽出する
    def filter_hosts_by_installed_package(self, package):
        df = self.df
        ins_PL = PackageList()
    ...
    def make_upgrade_package_inventory_and_playbook(self, package, post_upgrade_task="", service_name="", **kwargs):
        ...
        self.filter_hosts_by_installed_package(package)
...
class PackageList():
    ...
    def check_vulnerability(self, os_name, os_version, package, package_version, html_output_flg=True):
        # 標準出力の列幅を無制限に
        pd.set_option('display.max_colwidth', 1000)

        # check_vulnerabiiltyのvalidationチェックとpackage_version比較対象packageのリストアップ
        candidate_df = self._check_vulnerability_extract_target_package(os_name, os_version, package)
    ...
    def _check_vulnerability_extract_target_package(self, os_name, os_version, package):
        df = self.df
        pdf = df[df["os"]==os_name]
        if len(pdf.index) == 0:
            print(f"osが{os_name}のVMは存在しません\n管理対象OSは以下です：\n->{df['os'].drop_duplicates().tolist()}")
            return
        ppdf = pdf[pdf["os_version"]==os_version]
        if len(ppdf.index) == 0:
            print(f"バージョン{os_version}の{os_name}のVMは存在しません\n管理対象の{os_name}のVMのバージョンは以下です：\n ->{pdf['os_version'].drop_duplicates().tolist()}")
            return
        pppdf = ppdf[ppdf["name"]==package]
        if len(pppdf.index) == 0:
            print(f"管理している{os_name}{os_version}のVMに、{package}パッケージはインストールされていません\n導入済パッケージのリストは以下です：\n->")
